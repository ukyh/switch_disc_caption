...
def train(opt):
    ...
    assert opt.structure_after >= 0 or opt.self_critical_after >= 0, 'CE+RL optimization requires RL method'
    infos['opt'] = opt
    ...
    try:
        while True:
            ...
            model_out_ce = dp_lw_model(
                fc_feats, att_feats, labels, masks, att_masks, data['gts'], torch.arange(0, len(data['gts'])),
                sc_flag=False, struc_flag=False
            )

            loss_rl = model_out['loss'].mean()
            loss_ce = model_out_ce['loss'].mean()
            loss = opt.cerl_lambda * loss_rl + (1 - opt.cerl_lambda) * loss_ce
            ...
            print("Joint: iter {} (epoch {}), train_loss = {:.3f}, time/batch = {:.3f}" \
                .format(iteration, epoch, train_loss, end - start))
            if struc_flag:
                print("RL: iter {} (epoch {}), train_loss = {:.3f}, lm_loss = {:.3f}, struc_loss = {:.3f}, time/batch = {:.3f}" \
                    .format(iteration, epoch, rl_loss, model_out['lm_loss'].mean().item(), model_out['struc_loss'].mean().item(), end - start))
            else:
                print("RL: iter {} (epoch {}), avg_reward = {:.3f}, time/batch = {:.3f}" \
                    .format(iteration, epoch, model_out['reward'].mean(), end - start))
            print("CE: iter {} (epoch {}), train_loss = {:.3f}, time/batch = {:.3f}" \
                .format(iteration, epoch, ce_loss, end - start))
