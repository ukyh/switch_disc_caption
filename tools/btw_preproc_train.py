...
from captioning.utils.rewards import init_scorer, get_btw_score
...
def train(opt):
    ...
    # NOTE: lw and aw should be ignored in ciderbtw pre-computing
    opt.btw_lw = 0
    opt.btw_aw = -1
    infos['opt'] = opt
    ...
    try:
        while True:
            ...
            _gts = np.vstack(data['gts']) # np(batch * seq_per_img, 16)
            gts_btw_scores = get_btw_score(_gts, data['sim_gts'], opt, mode='weight') # np(batch * seq_per_img, 16)
            seq_per_img = len(gts_btw_scores) // len(data['infos'])
            for _idx, items in enumerate(data['infos']):    # {ix:[score * seq_per_img], ...}
                gts_btw_dict[items['ix']] = gts_btw_scores[_idx * seq_per_img: (_idx + 1) * seq_per_img][:, 0].tolist()
            
            # optimizer.zero_grad()
            # if opt.use_btw:
            #     model_out = dp_lw_model(
            #         fc_feats, att_feats, labels, masks, att_masks, data['gts'], torch.arange(0, len(data['gts'])),
            #         sc_flag, struc_flag, sim_gts=data['sim_gts']
            #     )
            # else:
            #     model_out = dp_lw_model(
            #         fc_feats, att_feats, labels, masks, att_masks, data['gts'], torch.arange(0, len(data['gts'])),
        ...
        assert len(gts_btw_dict) == len(loader.dataset.split_ix['train'])
        with open(opt.gts_btw_json, 'w') as outfile:
            json.dump(gts_btw_dict, outfile, indent=4)
        ...
        # print(stack_trace)
        print('## Warning: Save gts_btw on exception ...')
        with open(opt.gts_btw_json, 'w') as outfile:
            json.dump(gts_btw_dict, outfile, indent=4)
