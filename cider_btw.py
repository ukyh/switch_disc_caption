...
class CiderScorer(object):
    ...
    def __iadd__(self, other):
        ...
        if type(other) is tuple:
            ...
            self.crefs_btw.extend(other[2])
        else:
            self.ctest.extend(other.ctest)
            self.crefs.extend(other.crefs)
            self.crefs_btw.extend(other.crefs_btw)
    ...
    def compute_cider(self):
        ...
        scores = []
        for test, refs, refs_btw in zip(self.ctest, self.crefs, self.crefs_btw):
            # compute vector for test captions
            vec, norm, length = self.counts2vec(test)
            # compute vector for ref captions
            score = np.zeros((len(refs), self.n))   # np(len(refs), n)
            ...
            score_avg = np.mean(score, 1)   # np(len(refs))
            # divide by number of references
            score_avg = np.sum(score_avg * refs_btw) / len(refs)    # weight with CIDErBtw scores
...
class CiderBtw:
    """
    Main Class to compute the CIDEr-D Btw metric
    ...
    def compute_score(self, gts, res, gts_btw):
        """
        ...
        for res_id in res:
            ...
            ref_btw = gts_btw[res_id['image_id']]
            ...
            assert(len(ref) > 0)
            assert(type(ref_btw) is np.ndarray)
            assert(len(ref) == len(ref_btw))
            tmp_cider_scorer += (hypo[0], ref, ref_btw)
