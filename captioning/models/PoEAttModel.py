...
class AttModel(CaptionModel):
    ...
    def get_logprobs_state(self, it, fc_feats, att_feats, p_att_feats, att_masks, state, output_logsoftmax=1):
        ...
        xt = self.embed(it)
        with torch.no_grad():
            xt_base = self.base_model.embed(it)
        ...
        if output_logsoftmax:
            ...
            raise KeyError('Currently, only logsoftmax is supported')
            logprobs = self.logit(output)
        
        if not self.simple_ft:
            with torch.no_grad():
                output_base, state_base = self.base_model.core(xt_base, fc_feats, att_feats, p_att_feats, state_base, att_masks)
                if self.poe_temp == 1:
                    logprobs_base = F.log_softmax(self.base_model.logit(output_base), dim=1)
                else:
                    logprobs_base = F.log_softmax(self.base_model.logit(output_base) * self.poe_temp, dim=1)

            logprobs = F.log_softmax(logprobs + logprobs_base, dim=-1)

        if self.focal_loss != '':
            assert self.simple_ft == True, 'Focal loss is not used with PoE'
            if self.focal_loss == 'focal':
                logprobs = (1.0 - logprobs.exp()).pow(self.focal_gamma) * logprobs
            if self.focal_loss == 'antifocal':
                logprobs = (1.0 + logprobs.exp() * self.focal_anti_alpha).pow(self.focal_gamma) * logprobs
...
class PoEAtt2in2Model(AttModel):
    def __init__(self, opt):
        super(PoEAtt2in2Model, self).__init__(opt)
        self.core = Att2in2Core(opt)
        delattr(self, 'fc_embed')
        self.fc_embed = lambda x : x

        for param in self.att_embed.parameters():   # fix img_enc
            param.requires_grad = False
        for param in self.ctx2att.parameters():     # fix img_enc
            param.requires_grad = False
        for k, param in self.named_parameters():    # fix seq_enc
            if not k.startswith('logit'):
                param.requires_grad = False

        # base_model
        self.base_model = AttModel(opt)
        self.base_model.core = Att2in2Core(opt)
        delattr(self.base_model, 'fc_embed')
        self.base_model.fc_embed = lambda x : x
        for param in self.base_model.parameters():
            param.requires_grad = False
        
        self.simple_ft = getattr(opt, 'simple_ft', False)
        self.poe_temp = getattr(opt, 'poe_temp', 1)
        self.focal_loss = getattr(opt, 'focal_loss', '')
        self.focal_gamma = getattr(opt, 'focal_gamma', 1)
        self.focal_anti_alpha = getattr(opt, 'focal_anti_alpha', 1)
...
class PoEUpDownModel(AttModel):
    def __init__(self, opt):
        super(PoEUpDownModel, self).__init__(opt)
        self.num_layers = 2
        self.core = UpDownCore(opt)
        ...
        for param in self.ctx2att.parameters():     # fix img_enc
            param.requires_grad = False
        for param in self.fc_embed.parameters():    # fix img_enc (updown uses `self.fc_embed`)
            param.requires_grad = False
        for k, param in self.named_parameters():    # fix seq_enc
            if not k.startswith('logit'):
                param.requires_grad = False
        ...
        # base_model
        self.base_model = AttModel(opt)
        self.base_model.num_layers = 2
        self.base_model.core = UpDownCore(opt)
        for param in self.base_model.parameters():
            param.requires_grad = False
