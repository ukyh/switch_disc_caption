...
from .PoEnsAttModel import sort_pack_padded_sequence, pad_unsort_packed_sequence, pack_wrapper, AttModel
...
class Generator(nn.Module):
    ...
    def forward(self, x):
        # print("x", x.shape)
        # print("proj x", self.proj(x).shape)
        return F.log_softmax(self.proj(x), dim=-1)
    
    def forward_temp(self, x, poe_temp):
        return F.log_softmax(self.proj(x) * poe_temp, dim=-1)
...
class TransformerModel(AttModel):
    ...
    def __init__(self, opt):
        ...
        # NOTE: Fix att_embed to be compatible with model and base_model
        for param in self.att_embed.parameters():       # fix img_enc
            param.requires_grad = False
        for k, param in self.model.named_parameters():  # fix seq_enc
            if not k.startswith('generator'):
                param.requires_grad = False
        
        c = copy.deepcopy
        self.base_model = c(self.model)
    ...
    def logit_base(self, x): # unsafe way
        return self.base_model.generator.proj(x)
    ...
    def _prepare_feature_forward(self, att_feats, att_masks=None, seq=None):
        ...
        # with torch.no_grad():
        #     base_att_feats = pack_wrapper(self.base_att_embed, att_feats, att_masks)
    ...
    def _forward(self, fc_feats, att_feats, seq, att_masks=None):
        ...
        out = self.model(att_feats, seq, att_masks, seq_mask)   # Decoder -> output features
        outputs = self.model.generator(out) # Generator -> log_softmax

        if not self.simple_ft:
            with torch.no_grad():
                out_base = self.base_model(att_feats, seq, att_masks, seq_mask)
                if self.poe_temp == 1:
                    outputs_base = self.base_model.generator(out_base)
                else:
                    outputs_base = self.base_model.generator.forward_temp(out_base, self.poe_temp)

            outputs = F.log_softmax(outputs + outputs_base, dim=-1)
        
        if self.focal_loss != '':
            assert self.simple_ft == True, 'Focal loss is not used with PoE'
            if self.focal_loss == 'focal':
                outputs = (1.0 - outputs.exp()).pow(self.focal_gamma) * outputs
            if self.focal_loss == 'antifocal':
                outputs = (1.0 + outputs.exp() * self.focal_anti_alpha).pow(self.focal_gamma) * outputs
